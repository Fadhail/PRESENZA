    ...
    if ($result->num_rows == 1) {
        ...
        if (password_verify($password, $user['password'])) {
            ...
            if ($user['role'] == 'admin') {
                ...
                header("Location: user/presensi.php");
            }
            exit();
        } else {
    ...
    <div class="min-h-screen flex flex-col items-center justify-center py-6 px-4">
        <div class="max-w-md w-full">
            <div class="p-8 rounded-2xl bg-white shadow">
                ...
                <?php if (!empty($error)): ?>
                    <div class="text-red-600 text-sm text-center mb-4"><?php echo htmlspecialchars($error); ?></div>
                <?php endif; ?>
                <form method="POST" action="login.php" class="mt-8 space-y-4">
                    ...
                    <p class="text-gray-800 text-sm !mt-8 text-center">
                        Belum punya akun? <a href="register_form.php" 
                                             class="text-blue-600 hover:underline ml-1 whitespace-nowrap font-semibold">Register di sini</a>
                    </p>
                </form>
            </div>
        </div>
